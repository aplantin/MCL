...
#' @return List with intercept vector, beta matrix (q x (nlam1 x nlam2)), gamma matrix (p x (nlam1 x nlam2)), lambda sequence (list: lam1, lam2)
#' @export
#'
mcl_step <- function(Z, W, y, mu = 1, groups, step = 0.1, lam.seq = NULL, lam.max = NULL,
  ...
  for (i in 1:nlam[1]) {
    for (j in 1:nlam[2]) {
      ...
      res <- mclC_step(Z, W, y, facZ, facW, step, groups, groupIdx, mu,
