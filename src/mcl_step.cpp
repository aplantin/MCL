...
//' @export
// [[Rcpp::export]]
Rcpp::List mclC_step(arma::mat Z, arma::mat W, arma::vec y,
                arma::vec facZ, arma::vec facW, double step,
  ...
  while (count < maxit && diff > thresh) {
    ...
    for (int i = 0; i < q; i++) {
      ...
      beta(i) = old + step * (mu1 * softC(t1, lam1) - old);
    ...
    for (int j = 0; j < p; j++) {
      ...
      gamma(j) = old + step * (mu1 * softC(t2, lam2) - old);
...
//' @export
// [[Rcpp::export]]
arma::vec findMaxLams_step(arma::mat Z, arma::mat W, arma::vec y,
                      arma::vec facZ, arma::vec facW, double step,
                      arma::vec groups, Rcpp::List groupIdx,
  ...
  while (all_zero == true) {
    maxlam1 *= 0.95;
    Rcpp::List res = mclC_step(Z, W, y, facZ, facW, step, groups, groupIdx, mu,
